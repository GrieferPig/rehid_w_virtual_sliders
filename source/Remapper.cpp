...
static Result PMDBG_GetCurrentAppInfo(FS_ProgramInfo *outProgramInfo, u32 *outPid, u32 *outLaunchFlags)
{
    ...
    if (R_FAILED(ret = svcSendSyncRequest(*pmDbgGetSessionHandle())))
        return ret;
...
const u32 FN_VOL_UP = BIT(16);
const u32 FN_VOL_DN = BIT(17);
const u32 FN_3D_UP = BIT(18);
const u32 FN_3D_DN = BIT(19);
...
static uint32_t keystrtokeyval(char *str)
{
    ...
    static const key_s keys[] = {
        ...
        {"CSUP", KEY_CSTICK_UP},
        // dummy keys, used for calling corrsponding method only
        {"VOLUP", FN_VOL_UP},
        {"VOLDN", FN_VOL_DN},
        {"3DUP", FN_3D_UP},
        {"3DDN", FN_3D_DN},
...
uint32_t Remapper::Remap(uint32_t hidstate, uint32_t newpressedkeys)
{
    ...
    for (int i = 0; i < m_keyentries; i++)
    {
        ...
        {
            switch (m_remapkeyobjects[i].newkey)
            {
            case FN_3D_UP:
                incrementSliderVal();
                break;
            case FN_3D_DN:
                ...
                break;
            case FN_VOL_UP:
                incrementVolSliderVal();
                break;
            case FN_VOL_DN:
                decrementVolSliderVal();
                break;
            default:
...
uint32_t Remapper::CirclePadRemap(uint32_t hidstate, CirclePadEntry *circlepad)
{
    ...
    */
    ...
    circlepad->x = newentry.x;
    circlepad->y = newentry.y;
    return newstate;
}
Result Remapper::ReadConfigFile()
{
    ...
    if (ret) // Bindings for title not found, check if global profile is present
    {
        ...
        if (ret)
            return -1; // global profile not found, do not appply any settings.
        m_isglobal = true;
    }
